...
class HyperLora(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.training and all((r.item(), l.item()) in self.role_emb_tensor.all_lora_tensor for r, l in zip(role_ids, layer_ids)):
            hyper_A = [self.role_emb_tensor.all_lora_tensor[(r.item(), l.item())] for r, l in zip(role_ids, layer_ids)]
            hyper_A = torch.stack(hyper_A, dim=0).to(x.device)
            scale = self.alpha / self.rank
            out = self.linear(x)
            result_dtype = out.dtype
            x = x.to(self.lora_B.dtype)
            delta = self.dropout(x).transpose(0, 1).bmm(hyper_A)
            delta = delta @ self.lora_B * scale
            
            out = delta.transpose(0, 1) + out
        ...
        if not self.training:
            for r, l, h in zip(role_ids, layer_ids, hyper_A):
                if (r.item(), l.item()) not in self.role_emb_tensor.all_lora_tensor:
                    self.role_emb_tensor.all_lora_tensor[(r.item(), l.item())] = h
        out = self.linear(x)
        result_dtype = out.dtype
        x = x.to(self.lora_B.dtype)
        delta = self.dropout(x).transpose(0, 1).bmm(hyper_A)
...
class Chatglm2HyperNetwork(nn.Module):
    def __init__(self,config):
        print(f"dropout = {config.dropout}")
        super(Chatglm2HyperNetwork, self).__init__()
        self.config = config
        ...
        print(self.roles_emb.weight)
        print(self.layers_emb.weight)
        self.encoding_dim = config.roles_emb_dim + config.layers_emb_dim
        print("residual_blocks_num: ", self.residual_blocks_num)
        print("roles_emb_dim: ", self.roles_emb_dim)
        print("layers_emb_dim: ", self.layers_emb_dim)
        print("encoding_dim: ", self.encoding_dim)
        print("hyper_hidden_dim: ",self.hyper_hidden_dim)
        ...
        # query_key_value
        self.qkv_down_linear = LoRAHead(self.hyper_hidden_dim, config.hidden_size * config.rank_dim)
        # dense
        self.dense_down_linear = LoRAHead(self.hyper_hidden_dim, config.hidden_size * config.rank_dim)
        # dense_h_to_4h
        self.dense_h_to_4h_down_linear = LoRAHead(self.hyper_hidden_dim, config.hidden_size * config.rank_dim)
        # dense_4h_to_h
        self.dense_4h_to_h_down_linear = LoRAHead(self.hyper_hidden_dim, config.ffn_hidden_size * config.rank_dim)
    ...
    def hyper_init(self, layer, target_in, target_out):
        with torch.no_grad():
            ...
            layer.down.weight.data *= factor
            layer.up.weight.data *= factor
            return target_std
    ...
    def init_weights(self):
        ...
        self.hyper_init(self.qkv_down_linear, self.config.hidden_size, self.rank_dim)
        self.hyper_init(self.dense_down_linear, self.config.hidden_size, self.rank_dim)
        self.hyper_init(self.dense_h_to_4h_down_linear, self.config.hidden_size, self.rank_dim)
        self.hyper_init(self.dense_4h_to_h_down_linear, self.config.ffn_hidden_size, self.rank_dim)
